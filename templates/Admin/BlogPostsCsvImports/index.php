 ...
 * ブログ記事CSVインポート画面
 *
 * BcCsvImportCore の index.php をベースに、ブログ選択プルダウンを追加したもの。
 * adminBase をこのプラグインのコントローラーに向けている。
 *
...
<?php $this->BcBaser->css('BcCsvImportCore.admin/csv_import', ['block' => true]) ?>
        ...
        <tbody class="bca-table-listup__tbody">
          <?php foreach ($pendingJobs as $job): ?>
            <tr>
              ...
              <td class="bca-table-listup__tbody-td"><?= $job->mode === 'strict' ? __d('baser_core', '事前確認') : __d('baser_core', 'スキップ') ?></td>
              ...
              <td class="bca-table-listup__tbody-td bca-table-listup__tbody-td--actions">
                <?php if (!empty($job->can_resume)): ?>
                  <?= $this->BcAdminForm->button(__d('baser_core', '再開'), [
                    'type' => 'button',
                    'class' => 'bca-btn bca-actions__item js-resume-btn',
                    'data-token' => $job->job_token,
                    'data-total' => $job->total,
                    'data-processed' => $job->processed,
                    'data-phase' => $job->phase,
                    'data-mode' => $job->mode,
                    'data-import-strategy' => $job->import_strategy ?? 'append',
                ...
                <?= $this->BcAdminForm->button(__d('baser_core', '削除'), [
                  'type' => 'button',
                  'class' => 'bca-btn bca-actions__item js-delete-btn',
                  'data-token' => $job->job_token,
...
<section class="bca-section" data-bca-section-type="form-group" id="js-upload-section">
  ...
  $encodingLabels  = ['auto' => __d('baser_core', '自動判別（推奨）'), 'UTF-8' => 'UTF-8', 'Shift-JIS' => 'Shift-JIS'];
  $modeLabels      = ['strict' => __d('baser_core', '事前確認モード'), 'lenient' => __d('baser_core', 'スキップモード')];
  $strategyLabels  = ['append' => __d('baser_core', '追記'), 'replace' => __d('baser_core', '全件入れ替え')];
  $duplicateLabels = ['skip' => __d('baser_core', 'スキップ'), 'overwrite' => __d('baser_core', '上書き'), 'error' => __d('baser_core', 'エラー')];
  ?>
  <table class="form-table bca-form-table" data-bca-table-type="type2">
    <tbody>
      <tr>
        <th class="col-head bca-form-table__label">
          <?= $this->BcAdminForm->label('blog_content_id', __d('baser_core', 'インポート先ブログ') . ' <span class="bca-label" data-bca-label-type="required">' . __d('baser_core', '必須') . '</span>', ['escape' => false]) ?>
        </th>
        <td class="col-input bca-form-table__input">
          <?php if (!empty($blogOptions)): ?>
            ...
            <p class="bca-form__note">
              <?= __d('baser_core', 'インポートする記事を登録するブログを選択してください。') ?>
            </p>
          <?php else: ?>
            <p class="bca-form__note" style="color:#c33;">
              <?= __d('baser_core', '利用できるブログがありません。先にブログコンテンツを作成してください。') ?>
            </p>
            <?= $this->BcAdminForm->hidden('blog_content_id', ['value' => 0, 'id' => 'blog-content-id']) ?>
          <?php endif; ?>
        </td>
      </tr>
      <tr>
      ...
      <tr>
        ...
        <td class="col-input bca-form-table__input">
          ...
          <p class="bca-form__note">
            <a href="#" id="js-download-posts-btn" class="bca-btn bca-actions__item" data-bca-btn-type="download">
              <?= __d('baser_core', '選択中ブログの記事CSVダウンロード') ?>
            </a>
          </p>
          <p class="bca-form__note">
            <?= __d('baser_core', '選択中のブログに登録されている記事を、インポート形式に近いCSVでダウンロードします。') ?>
          </p>
        </td>
      </tr>
    </tbody>
  ...
  <?php if ($showOptionSection): ?>
    <div class="bca-collapse__action">
      <?= $this->BcAdminForm->button(
        __d('baser_core', 'オプション') . '&nbsp;&nbsp;<i class="bca-icon--chevron-down bca-collapse__btn-icon"></i>',
        [
          'type' => 'button',
  ...
  <div class="bca-actions">
    <div class="bca-actions__before"></div>
    <div class="bca-actions__main">
      <?= $this->BcAdminForm->button(__d('baser_core', 'アップロード開始'), [
        'type' => 'button',
        'id' => 'js-start-btn',
        'class' => 'bca-btn bca-actions__item',
...
<section class="bca-section" data-bca-section-type="form-group" id="js-result-section" style="display:none;">
  ...
  <table class="form-table bca-form-table" id="js-result-table" data-bca-table-type="type2">
    <tbody>
      <tr>
        <th class="col-head bca-form-table__label"><?= __d('baser_core', '処理件数') ?></th>
        <td class="col-input bca-form-table__input" id="js-res-total">-</td>
      </tr>
      <tr>
        <th class="col-head bca-form-table__label"><?= __d('baser_core', '成功件数') ?></th>
        <td class="col-input bca-form-table__input" id="js-res-success">-</td>
      </tr>
      <tr id="js-dup-row" style="display:none;">
        <th class="col-head bca-form-table__label"><?= __d('baser_core', '重複スキップ件数') ?></th>
        <td class="col-input bca-form-table__input" id="js-res-dup">-</td>
      </tr>
      <tr id="js-error-count-row" style="display:none;">
        <th class="col-head bca-form-table__label" style="color:#c33;"><?= __d('baser_core', 'エラー件数') ?></th>
        <td class="col-input bca-form-table__input" id="js-res-error" style="color:#c33;">-</td>
      </tr>
      <tr>
        <th class="col-head bca-form-table__label"><?= __d('baser_core', '処理時間') ?></th>
        <td class="col-input bca-form-table__input" id="js-res-time">-</td>
      </tr>
    </tbody>
  </table>
  ...
  <div class="bca-actions">
    <div class="bca-actions__before"></div>
    <div class="bca-actions__main">
      <?= $this->BcAdminForm->button(__d('baser_core', '新しいインポートを開始'), [
        'type' => 'button',
        'id' => 'js-restart-btn',
        'class' => 'bca-btn bca-actions__item',
      ]) ?>
